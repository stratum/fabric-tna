...
parser FabricParser (packet_in packet,
    ...
    state parse_packet_out_and_accept {
        ...
        packet.extract(hdr.ig.packet_out);
        transition accept;
    }
    ...
    state parse_fake_ethernet {
        packet.extract(hdr.ig.fake_ethernet);
        fake_ethernet_t tmp = packet.lookahead<fake_ethernet_t>();
        transition select(tmp.ether_type) {
            default: parse_ethernet;
        }
    ...
    state parse_ethernet {
        packet.extract(hdr.ig.ethernet);
    ...
    state parse_vlan_tag {
        packet.extract(hdr.ig.vlan_tag);
        // Initialize lookup metadata. Packets without a VLAN header will be
        // treated as belonging to a default VLAN ID
        fabric_md.ingress.bridged.base.vlan_id = hdr.ig.vlan_tag.vlan_id;
        // fabric_md.ingress.bridged.base.vlan_cfi = hdr.ig.vlan_tag.cfi;
        // fabric_md.ingress.bridged.base.vlan_pri = hdr.ig.vlan_tag.pri;
        transition select(packet.lookahead<bit<16>>()) {
    ...
    state parse_eth_type {
        packet.extract(hdr.ig.eth_type);
        transition select(hdr.ig.eth_type.value) {
    ...
    state parse_mpls {
        packet.extract(hdr.ig.mpls);
        fabric_md.ingress.bridged.base.mpls_label = hdr.ig.mpls.label;
        fabric_md.ingress.bridged.base.mpls_ttl = hdr.ig.mpls.ttl;
    ...
    state parse_ipv4 {
        packet.extract(hdr.ig.ipv4);
        fabric_md.ingress.routing_ipv4_dst = hdr.ig.ipv4.dst_addr;
        fabric_md.ingress.bridged.base.ip_eth_type = ETHERTYPE_IPV4;
        //ipv4_checksum.add(hdr.ig.ipv4);
        //fabric_md.ingress.ipv4_checksum_err = ipv4_checksum.verify();
        // Need header verification?
        transition select(hdr.ig.ipv4.protocol) {
            PROTO_TCP: parse_tcp;
            PROTO_UDP: parse_udp;
    ...
    state parse_icmp {
        packet.extract(hdr.ig.icmp);
        transition accept;
    }

    state parse_tcp {
        packet.extract(hdr.ig.tcp);
        transition accept;
    }

    state parse_udp {
        packet.extract(hdr.ig.udp);
        gtpu_t gtpu = packet.lookahead<gtpu_t>();
        transition select(hdr.ig.udp.dport, gtpu.version, gtpu.msgtype) {
    ...
    state parse_gtpu {
        packet.extract(hdr.ig.gtpu);
        transition select(hdr.ig.gtpu.ex_flag, hdr.ig.gtpu.seq_flag, hdr.ig.gtpu.npdu_flag) {
            (0, 0, 0): set_gtpu_only;
    ...
    state parse_gtpu_options {
        packet.extract(hdr.ig.gtpu_options);
        bit<8> gtpu_ext_len = packet.lookahead<bit<8>>();
        transition select(hdr.ig.gtpu_options.next_ext, gtpu_ext_len) {
    ...
    state parse_gtpu_ext_psc {
        packet.extract(hdr.ig.gtpu_ext_psc);
        fabric_md.ingress.bridged.base.encap_presence = EncapPresence.GTPU_WITH_PSC;
        transition select(hdr.ig.gtpu_ext_psc.next_ext) {
    ...
    state parse_vxlan {
        packet.extract(hdr.ig.vxlan);
        fabric_md.ingress.bridged.base.encap_presence = EncapPresence.VXLAN;
        transition parse_inner_ethernet;
    }

    state parse_inner_ethernet {
        packet.extract(hdr.ig.inner_ethernet);
        packet.extract(hdr.ig.inner_eth_type);
        transition select(hdr.ig.inner_eth_type.value) {
    ...
    state parse_inner_ipv4 {
        packet.extract(hdr.ig.inner_ipv4);
        //inner_ipv4_checksum.add(hdr.ig.inner_ipv4);
        //fabric_md.ingress.inner_ipv4_checksum_err = inner_ipv4_checksum.verify();
        transition select(hdr.ig.inner_ipv4.protocol) {
    ...
    state parse_inner_tcp {
        packet.extract(hdr.ig.inner_tcp);
        transition accept;
    }

    state parse_inner_udp {
        packet.extract(hdr.ig.inner_udp);
        transition accept;
    }

    state parse_inner_icmp {
        packet.extract(hdr.ig.inner_icmp);
        transition accept;
    }
...
control FabricDeparser(packet_out       packet,
    ...
    apply {
            ...
            packet.emit(hdr.ig.fake_ethernet);
            packet.emit(hdr.ig.packet_in);
            packet.emit(hdr.ig.ethernet);
            packet.emit(hdr.ig.vlan_tag);
#if defined(WITH_XCONNECT) || defined(WITH_DOUBLE_VLAN_TERMINATION)
            packet.emit(hdr.ig.inner_vlan_tag);
#endif // WITH_XCONNECT || WITH_DOUBLE_VLAN_TERMINATION
            packet.emit(hdr.ig.eth_type);
            packet.emit(hdr.ig.mpls);
            packet.emit(hdr.ig.ipv4);
            packet.emit(hdr.ig.ipv6);
            packet.emit(hdr.ig.tcp);
            packet.emit(hdr.ig.udp);
            packet.emit(hdr.ig.icmp);
            // in case we parsed a GTPU packet but did not decap it
            packet.emit(hdr.ig.gtpu);
            packet.emit(hdr.ig.gtpu_options);
            packet.emit(hdr.ig.gtpu_ext_psc);
            packet.emit(hdr.ig.vxlan);
            packet.emit(hdr.ig.inner_ethernet);
            packet.emit(hdr.ig.inner_eth_type);
            packet.emit(hdr.ig.inner_ipv4);
            packet.emit(hdr.ig.inner_tcp);
            packet.emit(hdr.ig.inner_udp);
            packet.emit(hdr.ig.inner_icmp);
    }
}
