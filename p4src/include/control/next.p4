...
control EgressNextControl (inout parsed_headers_t hdr,
    ...
    action pop_vlan() {
        ...
        // might go out of a tagged port hand hence hit an egress_vlan entry
        // with push_vlan action, we always treat recirculation ports as
        // untagged as it makes it easier to re-use them for other purposes such
        // as uplink recirculation in the SPGW pipeline. As a result, when
        // processing an INT mirror, we always strip the VLAN header from the
        // report's inner packet. That's fine since DeepInsight ignores VLAN
        // headers.
