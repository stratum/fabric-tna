...
control Next (inout parsed_headers_t hdr,
    ...
    action set_mpls_label(mpls_label_t label) {
        fabric_md.common.mpls_label = label;
    }
    ...
    action set_double_vlan(vlan_id_t outer_vlan_id, vlan_id_t inner_vlan_id) {
        ...
        fabric_md.common.push_double_vlan = true;
        fabric_md.common.inner_vlan_id = inner_vlan_id;
    }
    ...
    table hashed {
        key = {
            fabric_md.next_id         : exact @name("next_id");
            fabric_md.ipv4_dst_addr   : selector;
            fabric_md.ipv4_src_addr   : selector;
            fabric_md.common.ip_proto : selector;
            fabric_md.common.l4_sport : selector;
            fabric_md.common.l4_dport : selector;
        }
        actions = {
            output_hashed;
            routing_hashed;
    ...
    action set_mcast_group_id(MulticastGroupId_t group_id) {
        ...
        fabric_md.common.is_multicast = true;
...
control EgressNextControl (inout parsed_headers_t hdr,
    ...
    action pop_mpls_if_present() {
        ...
        hdr.eth_type.value = fabric_md.common.ip_eth_type;
    }

    @hidden
    action set_mpls() {
        hdr.mpls.setValid();
        hdr.mpls.label = fabric_md.common.mpls_label;
        hdr.mpls.tc = 3w0;
        hdr.mpls.bos = 1w1; // BOS = TRUE
        hdr.mpls.ttl = fabric_md.common.mpls_ttl; // Will Decrement after push.
    ...
    action push_vlan() {
        ...
        hdr.vlan_tag.vlan_id = fabric_md.common.vlan_id;
    }
    ...
    action push_inner_vlan() {
        ...
        hdr.inner_vlan_tag.cfi = 0;
        hdr.inner_vlan_tag.pri = 0;
        hdr.inner_vlan_tag.vlan_id = fabric_md.common.inner_vlan_id;
    ...
    apply {
        if (fabric_md.common.is_multicast
             && fabric_md.common.ingress_port == eg_intr_md.egress_port) {
            eg_dprsr_md.drop_ctl = 1;
        }

        if (fabric_md.common.mpls_label == 0) {
...
#ifdef WITH_DOUBLE_VLAN_TERMINATION
        if (fabric_md.common.push_double_vlan) {
            ...
            if (!egress_vlan.apply().hit) {
                ...
                if (fabric_md.common.vlan_id != DEFAULT_VLAN_ID) {
                    push_vlan();
                }
            }
#ifdef WITH_DOUBLE_VLAN_TERMINATION
