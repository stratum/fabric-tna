...
parser FabricIngressParser (packet_in  packet,
    ...
    state parse_mpls {
        ...
        transition select(packet.lookahead<bit<IP_VER_SIZE_BITS>>()) {
...
parser FabricEgressParser (packet_in packet,
    ...
    state set_cpu_loopback_egress {
        ...
        packet.advance(ETH_TYPE_SIZE_BYTES * 8);
        transition parse_ethernet;
    }
    ...
    state strip_gtpu_and_accept {
        packet.advance((IPV4_HDR_SIZE_BYTES + UDP_HDR_SIZE_BYTES + GTP_HDR_SIZE_BYTES) * 8);
        transition accept;
    }
