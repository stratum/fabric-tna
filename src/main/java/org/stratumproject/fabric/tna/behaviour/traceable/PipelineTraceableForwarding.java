...
class PipelineTraceableForwarding extends AbstractPipelineTraceableCtrl {
    ...
    public TrafficSelector augmentPacket(TrafficSelector packet, FabricTraceableMetadata metadata,
        ...
        if (flowEntry.table().equals(P4InfoConstants.FABRIC_INGRESS_FORWARDING_BRIDGING)) {
            ...
            if (log.isDebugEnabled()) {
                log.debug("Packet {} before bridging update", packetForPiTranslation.add(
                        piCriterionForTranslation.build()).build());
            }
            ...
            if (log.isDebugEnabled()) {
                log.debug("Packet {} after bridging selector", packetForPiTranslation.add(
                        piCriterionForTranslation.build()).build());
            }
        } else if (flowEntry.table().equals(P4InfoConstants.FABRIC_INGRESS_FORWARDING_ROUTING_V4)) {
            // Adds Routing table fields
            if (log.isDebugEnabled()) {
                log.debug("Packet {} before routing update", packetForPiTranslation.add(
                        piCriterionForTranslation.build()).build());
            }
            ...
            if (log.isDebugEnabled()) {
                log.debug("Packet {} after routing selector", packetForPiTranslation.add(
                        piCriterionForTranslation.build()).build());
            }
            ...
            if (log.isDebugEnabled()) {
                log.debug("Packet {} before mpls update", packetForPiTranslation.add(
                        piCriterionForTranslation.build()).build());
            }
            ...
            if (log.isDebugEnabled()) {
                log.debug("Packet {} after mpls selector", packetForPiTranslation.add(
                        piCriterionForTranslation.build()).build());
            }
        } else {
    ...
    private TrafficSelector getRoutingMatchingFields(TrafficSelector packet,
        ...
        if (piCriterion != null) {
            piCriterion.fieldMatches().forEach(piFieldMatch ->
                    piCriterionBuilder.addMatchField(piFieldMatch.fieldId(), piFieldMatch));
        }
