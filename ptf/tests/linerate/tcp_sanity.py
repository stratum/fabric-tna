...
class TcpSanity(QosBaseTest, TRexAstfTest):
    ...
    def calc_avg_bps (self, rates: list) -> float:
        sum = 0
        # ignore first 2 seconds for ramp-up
        for rate in rates[2:]:
           sum += rate

        return sum / len(rates[2:])
...
class TcpSanityUnshaped(TcpSanity):
    ...
    def create_profile(self) -> ASTFProfile: 
        ...
        info.tcp.no_delay_counter = 5 * 8000
    ...
    def runTest(self):
        ...
        # verify results
        avg_tx_gbps = self.calc_avg_bps(results[UNSHAPED_SENDER_PORT]["tx_bps"]) / 1_000_000_000
        avg_rx_gbps = self.calc_avg_bps(results[UNSHAPED_RECEIVER_PORT]["rx_bps"]) / 1_000_000_000
        self.assertAlmostEqual(
            avg_tx_gbps,
            avg_rx_gbps,
            delta=0.1,
            msg=f"Average receive and transmit rates were not equal; tx {avg_tx_gbps}, rx {avg_rx_gbps}"
        )
        self.assertGreaterEqual(
            avg_tx_gbps,
            35.0, # Gbps tput
            msg=f"Server did not transmit high enough throughput; sent {avg_tx_gbps} Gbps",
        )
        self.assertGreaterEqual(
            avg_rx_gbps,
            35.0, # Gbps tput
            msg=f"Client did not receive high enough throughput; received {avg_rx_gbps} Gbps",
        )


class TcpSanityShaped(TcpSanity):
    """
    This test generates a single flow of TCP traffic. The purpose of this test
    is to verify whether TRex can generate an iPerf-like single flow of TCP that
    can saturate a link on a switch with unshaped ports.
    ...
    def create_profile(self) -> ASTFProfile: 
        ...
        # define client behavior
        prog_c = ASTFProgram()
        prog_c.recv(len(REQUEST))
        prog_c.set_send_blocking(False) # continue to the next send while the queue has space 
        prog_c.set_var("var2",RESPONSE_LOOP-1); # set to loop-1 because there is another blocking send
        prog_c.set_label("a:");
        prog_c.send(response)
        prog_c.jmp_nz("var2","a:") # dec var2; if not zero, jump back to a: and send again
        prog_c.set_send_blocking(True) # back to blocking mode 
        prog_c.send(response)
        ...
        if r[1]==0:
          prog_s.recv(r[0])
            ...
            prog_s.set_var("var1",r[1]); # set var1 to number of loops
            prog_s.set_label("a:");
            prog_s.recv(r[0],True)
            prog_s.jmp_nz("var1","a:") # dec var1; if not zero, jump back to a: and receive again
            if r[2]:
               prog_s.recv(bsize*r[2])
    ...
    @autocleanup
    def runTest(self):
        self.push_chassis_config(with_shaping=True)
        ...
        self.assertGreaterEqual(
            avg_rx_gbps,
            1.0, # Gbps tput
            msg=f"Client did not receive expected throughput; received {avg_rx_gbps} Gbps",
        )
