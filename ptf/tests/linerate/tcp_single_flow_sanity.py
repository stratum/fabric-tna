...
class TcpSingleFlow(QosBaseTest, TRexAstfTest):
...
class TcpSingleFlowUnshaped(TcpSingleFlow):
    """
    ...
    @autocleanup
    def runTest(self):
        self.push_chassis_config(no_shaping=True)
...
class TcpSingleFlow1GShaped(TcpSingleFlow):
    """
    The purpose of this test is to verify that the switch correctly shapes an
    iPerf-like single flow of TCP traffic down to 1 Gbps.

    Note: reverses direction of traffic to account for 1G-shaped switch egress
    port sending to TRex port for server.
    """
    ...
    def create_profile(self) -> ASTFProfile: 
        ...
        prog_c.set_send_blocking (False)   # continue to the next send when the queue has space 
        prog_c.set_var("var2",RESPONSE_LOOP-1); # set var to loop-1 (there is another blocking send)
        prog_c.set_label("a:");
        ...
        if r[1]==0:
          prog_s.recv(r[0])
            ...
            prog_s.set_var("var1",r[1]); # 
            prog_s.set_label("a:");
            prog_s.recv(r[0],True)
            prog_s.jmp_nz("var1","a:") # dec var "var2". in case it is *not* zero jump a: 
            if r[2]:
        ...
        info.tcp.mss = 9000
        info.tcp.no_delay = 1
        info.tcp.rxbufsize = 1024*1024*1024
        info.tcp.txbufsize = 1024*1024*1024
    ...
    @autocleanup
    def runTest(self):
        self.push_chassis_config(no_shaping=False)
        ...
        # verify results
        self.assertAlmostEqual(
            self.calc_avg_bps(results[SHAPED_SENDER_PORT]["tx_bps"]) / 1000_000_000,
            3.7, # Gbps
            delta=0.01,
            msg="Client did not transmit the expected amount of average throughput",
        )
        self.assertAlmostEqual(
            self.calc_avg_bps(results[SHAPED_RECEIVER_PORT]["rx_bps"]) / 1000_000_000,
            1.0, # Gbps
            delta=0.01,
            msg="Server did not receive the expected amount of average throughput",
        )
